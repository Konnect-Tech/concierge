...
/** JsonWebToken 토큰 암호화 */
function jwt_encrypt(array $data, string $secret_key, string $secret_iv) : string{
	$secret_key = hash('sha256', $secret_key);
	$secret_iv = substr(hash('sha256', $secret_iv), 0, 32);
	return str_replace('=', '', base64_encode(openssl_encrypt(json_encode($data, JSON_UNESCAPED_UNICODE), "AES-256-CBC", $secret_key, 0, $secret_iv)));
}

/** JsonWebToken 토큰 복호화 */
function jwt_decrypt(string $payload, string $secret_key, string $secret_iv) : array{
	$secret_key = hash('sha256', $secret_key);
	$secret_iv = substr(hash('sha256', $secret_iv), 0, 32);
	return json_decode(openssl_decrypt(base64_decode($payload), "AES-256-CBC", $secret_key, 0, $secret_iv), true);
}

include $_SERVER["DOCUMENT_ROOT"] . "/pro_inc/mailer/Exception.php";
include $_SERVER["DOCUMENT_ROOT"] . "/pro_inc/mailer/PHPMailer.php";
include $_SERVER["DOCUMENT_ROOT"] . "/pro_inc/mailer/SMTP.php";
...
//	$mail->Port = 465;
//	$mail->Username = $mail->From = "noreply@konnect-cs.com";
//	$mail->Password = 'cwrq zrvi tipj meqe';
//	$mail->SMTPKeepAlive = true;
	$mail->Host = "smtp.gmail.com";
	$mail->Port = 465;
	$mail->Username = $mail->From = "noreply@konnect.finance";
	$mail->Password = 'qhhx fcou bluy webd';
	$mail->SMTPKeepAlive = true;

	$mail->isHTML($isHTML);
	$mail->CharSet = 'UTF-8';
	$mail->FromName = "KONNECT CONCIERGE";
...
function get_member_profile_img($member_idx): string{
	if($member_idx === null){
		return "default_profile.png";
	}

	$query = " SELECT profile_chg FROM member_info WHERE `idx` = '$member_idx' ";
	$result = mysqli_query($GLOBALS['gconnet'], $query);
	$row = $result->fetch_array();

	return $row['profile_chg'] ?? get_member_profile_img(null);
}

function generate_auth_key(string $table, string $memberType = 'A'): string{
	$memberType = strtoupper($memberType);

	$last_auth_sql = "SELECT max(idx) as midx FROM ".$table;
	$last_auth_query = mysqli_query($GLOBALS['gconnet'], $last_auth_sql);
	$last_auth_row = mysqli_fetch_array($last_auth_query);

	$generate_num = 1;
	if($last_auth_row['midx']){
		$generate_num = $last_auth_row['midx'] + 1;
	}

	if($generate_num >= 10 && $generate_num < 100){
		$key = "{$memberType}0000$generate_num";
	}elseif($generate_num >= 100 && $generate_num < 1000){
		$key = "{$memberType}000$generate_num";
	}elseif($generate_num >= 1000 && $generate_num < 10000){
		$key = "{$memberType}00$generate_num";
	}elseif($generate_num >= 10000 && $generate_num < 100000){
		$key = "{$memberType}0$generate_num";
	}elseif($generate_num >= 100000){
		$key = "$memberType$generate_num";
	}else{
		$key ="{$memberType}0000$generate_num";
	}
...
function uploadFile($HTTP_POST_FILES, $el_name, $el, $_P_DIR_FILE, $filename_org = ""){
	...
	if($_FILES[$el_name]['name']){
		...
		if(!move_uploaded_file($fromName, $toName)){
			...
			error_frame("UPLOAD_COPY_FAILURE1");
			exit;
		}
...
function uploadMultiFile($el_name, $_P_DIR_FILE, $index, $filename_org = ""){
	if($_FILES[$el_name]['name']){
		...
		if(!move_uploaded_file($fromName, $toName)){
			error_frame("UPLOAD_COPY_FAILURE2");
			exit;
		}
